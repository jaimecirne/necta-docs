...
Figure~\ref{fig:pdc_graph} visualizes these raw directional data. Following stimulus onset (represented by $T=0$), distinct causal spikes appear that define the functional state. Specifically, Striate cortex (A17) drove the initial response to extrastriate targets (e.g., A17$\to$TZ) with a PDC peaking at [INSERIR TEMPO DO PICO AQUI] ms relative to stimulus onset, reaching [INSERIR VALOR PDC AQUI] $\pm$ [INSERIR SD AQUI]. This data provides the empirical foundation to move beyond descriptive graph metrics, demonstrating that sensory information flow reconfigures rapidly during the active state, transitioning control from striate to extrastriate sources.
...
The dynamic visualizations confirm that the network continuously reconfigures its functional architecture during sensory ingestion. In the baseline state, striate cortex (A17) was established as the primary dominant Fonte node with the highest out-degree/out-flow ratio. Following stimulus onset, the visual dynome reconfigured rapidly. The Transition Zone (TZ) emerged as a massive Active Hub, increasing its Normalized Node Strength from [INSERIR VALOR BASELINE] to [INSERIR VALOR STIMULUS] ($p < 0.05$). This active modular hub behavior, shifting between Sink and Source states depending on the current epoch, provides direct empirical evidence supporting the TZ as a flexible, self-tuning modulator in the cortical hierarchy, routing inter-hemispheric communication pathways during active perception.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=\textwidth]{figuras/Dyna_alt2.pdf}
    \caption[Topological reconfiguration of the Visual Dynome]{Time-resolved graphs visualize the continuous shift in network functional architecture during sensory ingestion. Node size represents normalized Node Strength. Nodes colors: A17 (Striate-Blue), TZ (Transition Zone-Red), A18 (Extrastriate-Green). Note the emergence of the Transition Zone as a highly centralized and active moduladora hub, shifting its causality Sink/Source bimodal role rapidly across epochs.}
    \label{fig:dyna_graphs}
\end{figure}
